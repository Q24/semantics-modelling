...
class DialogEncoderDecoder(Model):
    ...
    def __setstate__(self, state):
        ...
        # set all shared theano variables to the stored values
        for key, value in save_dict.items():


            attr_split = key.split('.')

            if attr_split[-1] != 'container':
                continue

            current_var = self

            for idx in xrange(len(attr_split) - 1):
                try:
                    current_var = getattr(current_var, attr_split[idx])
                except AttributeError as ae:
                    print ae
                    current_var = None

            if not current_var:
                logging.debug('could not load %s'%(key.replace('.container', '')))
                continue

            logging.debug('Loading %s with shape %s' % (key.replace('.container', ''), str(value.data.shape)))
            np_data = numpy.array(value.data)
            current_var.set_value(np_data)

        print

    def __getstate__(self):
        '''
        saves the models parameters to the given filename.
        '''

        # contains everything to be saved. Save state for later initialization and also the statistics
        save_dict = {'state': self.state}


        if hasattr(self, 'timings'):
            save_dict['timings'] = self.timings

        # accumulate all the shared theano values for storage
        for key, value in self.__dict__.items():

            if hasattr(value, '__dict__'):
                self.collect_container_from_theano_variable(key, value, save_dict)

        return save_dict


    def collect_container_from_theano_variable(self, variable_name, variable, dict, indicator='', depth = 0):
        import theano.compile.function_module

        if type(variable) == theano.compile.function_module.Function:
            return
        ...
        if variable_name == 'container':
            dict[indicator] = variable.container
            return dict


        if hasattr(variable, 'container'):
            indicator = '%s.container' % indicator
            return self.collect_container_from_theano_variable('container', variable, dict, indicator, depth=depth+1)
        ...
        for key, value in variable.__dict__.items():
            ...
            if hasattr(value, '__dict__') or key == 'container':
                new_indicator = '%s.%s'%(indicator, key)
                self.collect_container_from_theano_variable(key, value, dict, new_indicator, depth=depth+1)
    ...
    def reverse_utterances_indices(self, indices_seq):
        indices_seq_reversed = [1]

        for idx, token_idx in enumerate(indices_seq):

            if token_idx == self.eos_sym and idx != 0:
                indices_seq_reversed.extend(indices_seq[len(indices_seq_reversed):idx][::-1] + [1])

        return indices_seq_reversed
    ...
    def build_encoder_function_old(self, test_values=None):
        if not hasattr(self, 'encoder_fn'):

            if test_values:
                self.x_data.tag.test_value = test_values[0]
                self.x_data_reversed.tag.test_value = test_values[1]
                self.x_max_length.tag.test_value = test_values[2]
    ...
    def build_encoder_function2(self):
        if not hasattr(self, 'encoder_fn2'):
            self.encoder_fn = theano.function(inputs=[self.x_data, self.x_data_reversed,
                             self.x_max_length],
                             outputs=[self.h, self.hs], on_unused_input='warn', name="encoder_fn")
    ...
    def build_encoder_function(self, test_values=None):
        if not hasattr(self, 'encoder_fn'):

            if self.add_latent_gaussian_per_utterance:
                reset_bool = self.add_latent_gaussian_per_utterance
                self.add_latent_gaussian_per_utterance = False
            else:
                reset_bool = False
            ...
            if self.bidirectional_utterance_encoder:
                ...
                if test_values:
                    self.x_data.tag.test_value = test_values['x_data']
            ...
            if self.add_latent_gaussian_per_utterance:
                self.encoder_fn = theano.function(inputs=[self.x_data, self.x_data_reversed, \
                                                          self.x_max_length], \
                                                  outputs=[h, hs, hs_complete], on_unused_input='warn',
                                                  name="encoder_fn")
                # self.encoder_fn = theano.function(inputs=[self.x_data, self.x_data_reversed, \
                #             self.x_max_length], \
                #             outputs=[h, hs_complete, hs_and_h_future, latent_utterance_variable_approx_posterior_mean], on_unused_input='warn', name="encoder_fn")
            ...
            if reset_bool:
                self.add_latent_gaussian_per_utterance = reset_bool

        return self.encoder_fn

    def build_response_encoder_function(self, automatic_matrix_transpose=False):
        if not hasattr(self, 'response_encoder_fn'):
            logging.debug('building lightweight response encoder function')
            x_data = T.imatrix('x_data')

            h = self.utterance_encoder.build_encoder(x_data, prev_state=self.ph)

            self.response_encoder_fn = theano.function(inputs=[x_data],
                                                           outputs=[h], on_unused_input='ignore',
                                                           name="response_encoder_fn")
        return self.response_encoder_fn


    def build_single_step_encoder_function(self):
        if not hasattr(self, 'single_step_encoder_fn'):
            logging.debug('building single step context encoder function')
            #input_context_vec = T.fvector()
            #input_response_vec = T.fvector()
            input_context_vec = T.fvector()
            input_response_vec = T.fvector()

            x_mask = theano.shared(np.array(np.zeros((1,), dtype='float32'),dtype=theano.config.floatX))

            if self.dialogue_encoder_gating == "GRU":
                _res = self.dialog_encoder.GRU_dialogue_step(input_response_vec, x_mask, input_context_vec)
            else:
                _res = self.dialog_encoder.plain_dialogue_step(input_response_vec, x_mask, input_context_vec)

            self.single_step_encoder_fn = theano.function(inputs=[input_response_vec, input_context_vec],
                                          outputs=[_res[0]], on_unused_input='ignore',
                                          name="encoder_fn")

        return self.single_step_encoder_fn
    ...
    def __init__(self, state):
        ...
        if self.end_sym_utterance not in self.str_to_idx:
           raise Exception("Error, malformed dictionary! %s not in the dictionary"%self.end_sym_utterance)
        ...
        #TODO DEBUG CODE
        if 'test_batch' in state:
            batch = state['test_batch']

            self.x_data.tag.test_value = batch['x']
            self.x_data_reversed.tag.test_value = batch['x_reversed']
            self.x_max_length.tag.test_value = batch['max_length']
            self.x_cost_mask.tag.test_value = batch['x_mask']
            self.x_reset_mask.tag.test_value = batch['x_reset']
            self.ran_cost_utterance.tag.test_value = batch['ran_var_constutterance']
            self.x_dropmask.tag.test_value = batch['ran_decoder_drop_mask']
        ...
        if state['test_values_enabled']:
            ...
            train_iterator, _ = data_iterator.get_train_iterator(self.state)
            train_iterator.start()
        ...
        if self.initialize_from_pretrained_word_embeddings == True and os.path.exists(self.pretrained_word_embeddings_file):
