...
class CostCalculator(Sampler):


    def __init__(self, model):
        Sampler.__init__(self, model)
        self.name = 'CostCalculator'
        self.hyp_rec = 0
    ...
    def set_response(self, response):
        ...
        if isinstance(response, basestring):
            splitted = response.strip().split()
            response = self.model.words_to_indices(splitted)

        if response[-1] is not self.model.eos_sym:
            response = append(response, self.model.eos_sym)

        self.response = response
        #print 'transformed r ', response
        self.idx = 0

    def select_next_words(self, next_costs, next_probs, step_num, how_many):
        if self.idx >= len(self.response):
            word_id = self.model.eos_sym
        else:
            word_id = self.response[self.idx]

        if next_costs.flatten()[word_id] == float('inf'):
            print word_id
            print next_costs.flatten()[word_id]
            print next_costs.flatten()[word_id]

        self.idx += 1
        return ([0], [word_id]), [next_costs.flatten()[word_id]]
...
class BeamSampler(Sampler):
    ...
    def select_next_words(self, next_costs, next_probs, step_num, how_many):
        ...
        ret = numpy.unravel_index(args, next_costs.shape), flat_next_costs[args]
